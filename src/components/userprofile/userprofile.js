    ...
    loading===true || userLoading===true ? <Loader/> : (
...

        {
            posts && posts.length > 0 ? posts.map(post=>(
                <Post key={post._id} caption={post.caption} postId={post._id} likes={post.likes} comments={post.comments} ownerName={post.owner.name} ownerId={post.owner._id}/>
        ...
        <div className="accountright">
            ...
            {
                myProfile ? null : <Button variant="contained" style={{background: following ? "red" : ""}} onClick={followHandler} disabled={followLoading}>{following ? "Unfollow" : "Follow"}</Button>
            }
